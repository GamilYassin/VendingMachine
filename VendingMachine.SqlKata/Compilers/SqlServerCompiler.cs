namespace VendingMachine.QueryBuilder
{
    public class SqlServerCompiler : Compiler
    {
        ...
        protected override string CompileColumns(SqlResult ctx)
        {
            ...
            // If there is a limit on the query, but not an offset, we will add the top clause to
            // the query, which serves as a "limit" type clause within the SQL Server system similar
            // to the limit keywords available in MySQL.
        ...
        public override string CompileLimit(SqlResult ctx)
        {
            if (UseLegacyPagination)
            {
                // in legacy versions of Sql Server, limit is handled by TOP and ROW_NUMBER techniques
                return null;
            }
