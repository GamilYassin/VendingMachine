...
namespace VendingMachine.QueryBuilder
{
    public partial class Query
    {
        ...
        {
            // If the value is "null", we will just assume the developer wants to add a Having null
            // clause to the query. So, we will allow a short-cut here to that method for
            // convenience so the developer doesn't have to check.
            if (value == null)
            {
        ...
        {
            // If the developer has passed a string most probably he wants List<string> since string
            // is considered as List<char>
            if (values is string)
            {
